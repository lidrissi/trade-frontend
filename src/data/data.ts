...
export async function getOrder(number: string) {
  ...
  if (!order) {
    ...
    console.warn(
      `Order with number ${number} not found. Returning the first order as a fallback.`,
    );
    order = allOrders[0];
  }
...
export async function getBlogPostsByHandle(handle: string) {
  ...
  if (!post) {
    ...
    console.warn(
      `Post with handle ${handle} not found. Returning the first post as a fallback.`,
    );
    post = posts[0];
  }
...
export async function getCollections() {
  return [
    // default collections 1 - 7
    {
      ...
      handle: "jackets",
      description:
        "Explore our collection of trendy jackets that elevate your outfit.",
    ...
    },
    {
      ...
      description:
        "Casual t-shirts for everyday wear, combining comfort and style effortlessly. ",
      image: {
    ...
    },
    {
      ...
      sortDescription: "Best sellers",
      description:
        "Trendy jeans for a casual yet stylish look. Perfect for any occasion. ",
      image: {
...
export async function getCollectionByHandle(handle: string) {
  ...
  if (handle === "all") {
    return {
      ...
      handle: "all",
      description:
        "Explore our entire collection of products, from clothing to accessories.",
...
export async function getProductDetailByHandle(handle: string) {
  ...
  return {
    ...
    selectedOptions: [
      {
        name: "Color",
        value: product?.options.find((option) => option.name === "Color")
          ?.optionValues[1].name,
      },
      {
        name: "Size",
        value: product?.options.find((option) => option.name === "Size")
          ?.optionValues[0].name,
      },
    ],
    features: [
